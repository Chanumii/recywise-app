...
# Load environment variables from a project-root .env file before any os.environ reads, ensuring GEMINI_API_KEY is available at import time.
load_dotenv()
...
# ─────────────────────────────────────────────────────────────────
# ENDPOINT: ESTIMATE MATERIALS
# ─────────────────────────────────────────────────────────────────

@router.post("/api/estimate_materials")
def estimate_materials(vehicle: MaterialRequest):
    """
    Estimate material composition (% by weight) for a given vehicle.

    Primary   : Gemini 1.5 Flash with structured JSON output.
    Fallback  : Rule-based heuristics (age + make/model keyword matching).
    """
    try:
    ...
    # ── Primary: Gemini AI structured JSON ───────────────────────
    if gemini_client:
        try:
