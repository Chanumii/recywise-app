...
def _pgm_grams(fuel: str, disp: float, year: int) -> dict:
    ...
    """
    if fuel == "Diesel":
        # Diesel oxidation catalysts: platinum-high, palladium-lean.
        pt_per_l, pd_per_l, rh_per_l = 2.8, 0.10, 0.15
    elif fuel == "Hybrid":
        # Hybrid three-way catalysts: higher palladium.
        pt_per_l, pd_per_l, rh_per_l = 1.0, 2.00, 0.20
    elif fuel == "Electric":
        # Battery-electric vehicles:no catalytic converter.
...
def compute_v_cat_dynamic(fuel: str, disp: float, year: int, market_data: dict) -> float:
    ...
    Formula:
        gross = (Pt_g x pt_price) + (Pd_g x pd_price) + (Rh_g x rh_price)
        net   = max(gross x PGM_RECOVERY_FACTOR, 40.0)
    ...
    # Rhodium: use a conservative average ($4,500/troy oz) 
...
def _resolve_labour_cost(
    ...
    Formula: labour_cost = (resolved_time / 60) x effective_rate
    """
...
def _action_revenue(action: dict, features: dict) -> float:
    ...
    Revenue type tokens and their formulas:
        ...
        w_trans_steel -> w_trans x m_steel x 0.6     (steel scrap, partial)
        w_cop_cop     -> w_cop x m_cop               (copper scrap)
        w_whl_alum    -> w_whl x m_alum              (alloy scrap)
        v_batt      -> features["v_batt"]           (battery value)
        v_ref        -> features["v_ref"]            (refrigerant fee)
        w_rad_alum    -> w_rad x m_alum              (radiator scrap)
        w_pnl_steel   -> w_pnl x m_steel             (panel scrap)
        v_tyre_4    -> v_tyre x 4                  (per-wheel x 4)
        w_gls_gls     ->  w_gls x m_gls              (glass cullet)
        w_body_steel  -> w_body x m_steel             (hull scrap)
...
def build_explanation(
    ...
    """
    Generate a plain-language explanation for each ranked step.
    """
    name = action["name"]
    flags = flags or {}

    # Use the operator-configured time where available;
